...
import { Server } from "socket.io";
import { createRoom, getMessages, getRooms, storeMessage, } from "./src/appwrite/appwrite_server";
const dev = process.env.NODE_ENV !== "production";
const hostname = process.env.HOSTNAME || "localhost";
const port = parseInt(process.env.PORT || "3000", 10);
// async function loadAppwriteFunctions() {
...
async function startServer() {
    // const { createRoom, storeMessage, getRooms, getMessages } =
    //   await loadAppwriteFunctions();
    const app = next({ hostname, dev, port });
    const handler = app.getRequestHandler();
    await app.prepare();
    const httpServer = createServer(handler);
